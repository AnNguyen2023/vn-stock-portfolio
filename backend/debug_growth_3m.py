...
db = SessionLocal()
try:
    print("--- DEBUGGING GROWTH SERIES (3M) ---")
    result = growth_series(db, period="3m")
    
    print(f"Base Date: {result.get('base_date')}")
    print(f"Base NAV: {result.get('base_nav')}")
    ...
    if series:
        ...
        # Count non-zero
        non_zero = [x for x in series if x['PORTFOLIO'] != 0]
        print(f"Non-zero points: {len(non_zero)} / {len(series)}")
    else:
