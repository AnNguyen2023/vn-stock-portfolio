...
def deposit_money(req: schemas.DepositRequest, db: Session = Depends(get_db)):
    ...
    db.commit()
    # Xóa cache hiệu suất vì dòng tiền đã thay đổi
...
def get_performance(db: Session = Depends(get_db)):
    ...
    curr_stock_val = sum((Decimal(str(prices.get(h.ticker, {}).get("price", 0) if isinstance(prices.get(h.ticker), dict) else prices.get(h.ticker, 0))) or h.average_price) * h.total_volume for h in holdings)
    curr_nav = asset.cash_balance + curr_stock_val

    # Hàm tính % tăng trưởng chuẩn
    def calc_pct(days_ago=None, ytd=False):
        ...
        net_flow = sum((f.amount if f.type == models.CashFlowType.DEPOSIT else -f.amount) 
    ...
    # Lưu cache và đẩy task cho Worker
    try:
    ...
    def get_nav_history(limit: int = 20, db: Session = Depends(get_db)):    
        ...
        for i in range(len(snaps)-1):
            ...
            res.append({"date": curr.date.strftime('%Y-%m-%d'), "nav": float(curr.total_nav), "change": float(change), "pct": float(pct)})
        return res
    return result
# =========================================================================================
# --- HÀM TÍNH TOÁN TWRR (LÃI TRỌN ĐỜI/CHU KỲ) ---
def calculate_twrr_for_period(curr_nav, old_nav, net_flow):
    profit = curr_nav - (old_nav + net_flow)
    denom = old_nav + net_flow
    pct = (profit / denom * 100) if denom > 0 else 0
    return float(profit), float(pct)
# =========================================================================================
