    ...
    <div className="fixed inset-0 bg-slate-900/40 flex items-center justify-center p-4 z-[110] backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-white rounded-3xl p-8 w-full max-w-sm shadow-2xl border border-blue-100 transform animate-in zoom-in-95">
        ...
        <div className="flex gap-3">
          <button onClick={() => setShowUndoConfirm(false)} className="flex-1 py-4 bg-slate-100 text-slate-500 font-medium rounded-2xl text-sm uppercase hover:bg-slate-200 transition-all">Hủy bỏ</button>
          <button onClick={confirmUndo} className="flex-1 py-3 bg-blue-600 text-white font-medium rounded-2xl text-sm uppercase shadow-lg shadow-blue-100 active:scale-95 transition-all">Xác nhận</button>
        </div>
      </div>
    </div>
  );
