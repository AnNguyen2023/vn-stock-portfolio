...
export default function WLRenameModal({ isOpen, onClose, onRename, initialName }) {
    ...
    return (
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-[130] flex items-center justify-center p-4">
            <div className="bg-white rounded-[32px] p-8 w-full max-w-sm shadow-2xl animate-in zoom-in-95 duration-200 border border-slate-100">
                <h3 className="text-lg font-bold text-slate-700 uppercase mb-6 tracking-tight">Đổi tên danh sách</h3>
                ...
                <div className="flex gap-4">
                    ...
                    <button onClick={handleRename} className="flex-1 py-4 bg-[#00b894] text-white font-medium rounded-2xl text-sm uppercase hover:bg-[#00a383] transition-all shadow-lg shadow-emerald-100">Cập nhật</button>
                </div>
            </div>
        </div>
    );
