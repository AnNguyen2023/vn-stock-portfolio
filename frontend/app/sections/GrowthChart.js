...
// Bảng màu cố định cho biểu đồ đường
const COLORS = [
...
export default function GrowthChart({ 
  ...
  return (
    <div className="mb-10 bg-white rounded-2xl shadow-xl border border-slate-200 overflow-visible relative z-10">
      
      {/* --- HEADER BIỂU ĐỒ --- */}
      <div className="p-4 bg-slate-50/50 border-b border-slate-100 flex flex-col md:flex-row justify-between md:items-center gap-4">
         <h2 className="text-slate-800 text-sm font-bold uppercase tracking-wider flex items-center gap-2">
            <TrendingUp size={18} className="text-emerald-500"/> Tăng trưởng (%)
         </h2>

         {/* Bộ chọn mốc thời gian */}
         {/* Bộ chọn mốc thời gian: Nâng lên 12px, font bold thanh mảnh */}
         <div className="flex bg-white border border-slate-200 p-1 rounded-lg shadow-sm">
            {['1m', '3m', '6m', '1y'].map((range) => (
              <button 
                key={range} 
                onClick={() => setChartRange(range)} 
                className={`px-4 py-1.5 text-[12px] font-bold rounded-md uppercase transition-all tracking-wide ${
                  chartRange === range ? 'bg-emerald-500 text-white shadow-sm' : 'text-slate-400 hover:bg-slate-50'
                }`}
              >
                {range}
              </button>
         ...
         {/* Nút So sánh: Nâng lên 12px, đồng bộ font */}
         <div className="relative">
            ...
            {isDropdownOpen && (
              <div className="absolute right-0 top-full mt-2 w-56 bg-white rounded-xl shadow-2xl border border-slate-100 p-3 z-50 animate-in fade-in zoom-in duration-200">
                ...
                <div className="pt-2 mt-2 border-t border-slate-50 text-right">
                   <button onClick={() => setIsDropdownOpen(false)} className="text-[10px] font-black text-emerald-600 uppercase">Đóng</button>
                </div>
              </div>
            )}
      ...
      <div className="p-6 h-[350px] w-full">
        {chartData.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full space-y-3">
            ...
            <p className="text-slate-400 text-[11px] font-bold animate-pulse uppercase">Đang đồng bộ dữ liệu...</p>
          </div>
        ) : (
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={chartData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
              ...
              <YAxis axisLine={false} tickLine={false} tick={{fill: '#64748b', fontSize: 10, fontWeight: 700}} tickFormatter={(val) => `${val > 0 ? '+' : ''}${val}%`} />
              ...
              <Legend wrapperStyle={{paddingTop: '20px', fontSize: '11px', fontWeight: 800}} iconType="circle" />
              {selectedComparisons.map((ticker) => {
                if (ticker === 'PORTFOLIO') return <Line key="PORTFOLIO" type="monotone" dataKey="PORTFOLIO" name="Danh mục" stroke="#2563eb" strokeWidth={3} dot={false} activeDot={{r: 6, strokeWidth: 0}} />;
                if (ticker === 'VNINDEX') return <Line key="VNINDEX" type="monotone" dataKey="VNINDEX" name="VN-Index" stroke="#94a3b8" strokeWidth={2} dot={false} strokeDasharray="5 5" strokeOpacity={0.7} />;
                const stockOnlyList = selectedComparisons.filter(x => x!=='PORTFOLIO' && x!=='VNINDEX');
                const colorIdx = stockOnlyList.indexOf(ticker);
                return <Line key={ticker} type="monotone" dataKey={ticker} name={ticker} stroke={COLORS[colorIdx % COLORS.length]} strokeWidth={2} dot={false} activeDot={{r: 4, strokeWidth: 0}} />;
              })}
            </LineChart>
          </ResponsiveContainer>
        )}
